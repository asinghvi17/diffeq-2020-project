...
\[
    \phi(\epsilon) = J(y + \epsilon \cdot \eta) = \int_0^T F(t, y + \epsilon \eta, \dot y + \epsilon \dot \eta) ~ dt
\]

We can evaluate the \emph{derivative} of $\phi$ at 0.  We know, however, that there must be a stationary point at $\epsilon = 0$, which leads us to the inevitable conclusion that we must relinquish ourselves to the greedy clutches of algebra, and evaluate $\frac{d\phi}{d\epsilon}$ at 0.
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[every node/.style = {draw, outer sep = 0pt, thick}]
        ...
        \draw [-latex,ultra thick] (M.east) ++ (0.2cm,0) -- +(1cm,0) node [right, draw = none,] {$x$};
...
\begin{figure}[ht!]
    ...
    \caption{A timeseries view of the three-spring, two-mass system.}
    \label{fig: tstm}
\end{figure}
